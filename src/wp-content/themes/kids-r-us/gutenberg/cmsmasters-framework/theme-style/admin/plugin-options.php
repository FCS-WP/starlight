...
function kids_r_us_sidebar_wide_full_meta_fields($custom_all_meta_fields) {
		...
		foreach ($custom_all_meta_fields as $custom_all_meta_field) {
			if ($custom_all_meta_field['id'] == 'cmsmasters_layout') {
				if(isset($custom_all_meta_field['desc'])) {
					$custom_all_meta_field['desc'] = $custom_all_meta_field['desc'] . '</br>' . esc_html__('Wide width and Full width options in Block Editor work only for posts without sidebars.', 'kids-r-us');
				}
...
add_filter('get_custom_all_meta_fields_filter', 'kids_r_us_sidebar_wide_full_meta_fields');