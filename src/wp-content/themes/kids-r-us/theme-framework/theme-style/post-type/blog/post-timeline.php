...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_post_timeline'); ?>>
	...
	if ($date) {
		echo '<div class="cmsmasters_post_info entry-meta">';
			...
			kids_r_us_get_post_date('page', 'timeline');
	...
	<div class="cmsmasters_timeline_margin">
		...
		<?php
			if ($cmsmasters_post_format == 'image') {
				...
				kids_r_us_post_type_slider(get_the_ID(), $cmsmasters_post_images, 'post-thumbnail', $slider_data);
			} elseif ($cmsmasters_post_format == '' && !post_password_required() && has_post_thumbnail()) {
				kids_r_us_thumb(get_the_ID(), 'post-thumbnail', true, false, true, false, true, true, false);
			}
			
			
			kids_r_us_post_heading(get_the_ID(), 'h3');
