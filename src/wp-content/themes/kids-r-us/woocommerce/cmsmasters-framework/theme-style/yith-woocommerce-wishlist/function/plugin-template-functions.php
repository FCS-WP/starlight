...
function kids_r_us_add_wishlist_button($show = true) {
	global $product;
	...
	$popup_title = apply_filters('yith_wcwl_add_to_wishlist_popup_title', __('Select a Wishlist', 'kids-r-us'));
	...
	$label_add_text = esc_html(get_option('yith_wcwl_add_to_wishlist_text'));
	
	$label_browse_text = esc_html(get_option('yith_wcwl_browse_wishlist_text'));
		...
		if (!($disable_wishlist && ! is_user_logged_in())):
			$out .= '<div class="yith-wcwl-add-button ' . (($exists && ! $available_multi_wishlist) ? 'hide': 'show') . '" style="display:' . (($exists && ! $available_multi_wishlist) ? 'none': 'block') . '">';
				if (!$available_multi_wishlist) {
					$out .= '<a href="' . esc_url(add_query_arg('add_to_wishlist', $product_id)) . '" rel="nofollow" data-product-id="' . $product_id . '" data-product-type="' . $product_type . '" class="button_to_cart ' . (($label_add_text) ? '' : 'empty ') . 'cmsmasters_theme_icon_like ' . $link_classes . '" title="' . (($label_add_text) ? $label_add_text : esc_attr__('Add to Wishlist', 'kids-r-us')) . '">' . 
						'<span>' . (($label_add_text) ? $label_add_text : esc_html__('Add to Wishlist', 'kids-r-us')) . '</span>' . 
					'</a>' . 
					...
					if ($show_exists && $exists): 
						...
						$out .= '<a href="#add_to_wishlist_popup_' . $product_id . '_' . $unique_id . '" rel="nofollow" class="button cmsmasters_theme_icon_like ' . $link_classes . ' open-pretty-photo" data-rel="prettyPhoto[add_to_wishlist_' . $product_id . '_' . $unique_id . ']" ></a>' . 
								...
								'<div class="yith-wcwl-popup-content">' . 
									'<div class="yith-wcwl-first-row">' . 
										'<div class="yith-wcwl-wishlist-select-container">' . 
											...
											'<select name="wishlist_id" class="wishlist-select">' . 
												'<option value="0">' . apply_filters('yith_wcwl_default_wishlist_name', __('My Wishlist', 'kids-r-us')) . '</option>';
													if(!empty($lists)): 
												...
												$out .= '<option value="new">' . apply_filters('yith_wcwl_create_new_list_text', __('Create a new list', 'kids-r-us')) . '</option>' . 
											'</select>' . 
										'</div>' . 
									...
									'<div class="yith-wcwl-second-row">' . 
										'<div class="yith-wcwl-popup-new">' . 
											'<label for="wishlist_name">' .  apply_filters('yith_wcwl_new_list_title_text', __('Wishlist name', 'kids-r-us')) . '</label>' . 
										...
										'<div class="yith-wcwl-visibility">' . 
											'<select name="wishlist_visibility" class="wishlist-visibility">' . 
												'<option value="0" class="public-visibility">' . apply_filters('yith_wcwl_public_wishlist_visibility', __('Public', 'kids-r-us')) . '</option>' . 
												'<option value="1" class="shared-visibility">' . apply_filters('yith_wcwl_shared_wishlist_visibility', __('Shared', 'kids-r-us')) . '</option>' . 
												'<option value="2" class="private-visibility">' . apply_filters('yith_wcwl_private_wishlist_visibility', __('Private', 'kids-r-us')) . '</option>' . 
											'</select>' . 
										'</div>' . 
									'</div>' . 
								'</div>' . 
			...
			'<div class="yith-wcwl-wishlistaddedbrowse hide" style="display:none;">' . 
				'<a href="' . esc_url($wishlist_url) . '" class="button button_to_cart browse_wishlist cmsmasters_theme_icon_like' . (($label_browse_text) ? '' : ' empty') . '" rel="nofollow" title="' . (($label_browse_text) ? $label_browse_text : esc_attr__('Browse Wishlist', 'kids-r-us')) . '">' . 
					'<span>' . (($label_browse_text) ? $label_browse_text : esc_html__('Browse Wishlist', 'kids-r-us')) . '</span>' . 
				'</a>' . 
			'</div>' . 
			...
			$out .= '<a href="' . esc_url(add_query_arg(array( 'wishlist_notice' => 'true', 'add_to_wishlist' => $product_id ), get_permalink(wc_get_page_id('myaccount')))) . '" rel="nofollow" class="' . (($label_browse_text) ? '' : 'empty ') . 'button_to_cart ' . str_replace('add_to_wishlist', '', $link_classes) . '" title="' . (($label_browse_text) ? $label_browse_text : esc_attr__('Browse Wishlist', 'kids-r-us')) . '">' . 
					'<span>' . (($label_browse_text) ? $label_browse_text : esc_html__('Browse Wishlist', 'kids-r-us')) . '</span>' . 
			'</a>';
		endif;
...
/* Wishlist Count */
function kids_r_us_shop_wishlist_count() {
	$wishlist_url = YITH_WCWL()->get_wishlist_url();
	
	$output = '<div class="cmsmasters_wishlist_wrap">' . 
		'<a href="' . esc_url($wishlist_url) . '" class="cmsmasters_theme_icon_like" rel="nofollow"></a>' . 
	'</div>';
	
	echo kids_r_us_return_content($output);
}
