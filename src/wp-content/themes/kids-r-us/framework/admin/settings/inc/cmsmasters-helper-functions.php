...
function kids_r_us_get_admin_page() {
	global $pagenow;
...
function kids_r_us_default_tab() {
	$current_page = kids_r_us_get_admin_page();
...
function kids_r_us_get_the_tab() {
	global $pagenow;
	
	$default_tab = kids_r_us_default_tab();
...
function kids_r_us_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'kids-r-us') . '</a>';
		}
